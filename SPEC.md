...
1. 採買只記錄日期，因此購入批次從 `Asia/Taipei` 該日 00:00 起視為可用；序列化時間不代表實際購買時刻。
2. 清除既有 `ConsumptionAllocation` 與所有衍生餘額。
3. 依食材分組建立購入批次；批次依購入日、`createdAt`、ID 排序。
4. 將餐點用量與庫存調減合併成消耗事件，依 `occurredAt`、`createdAt`、ID 排序。
5. 每個消耗事件從當時已購入且仍有餘量的最早批次開始扣除。
6. 扣除部分批次時，成本依該批次當下剩餘成本與剩餘數量比例計算並四捨五入至分。
7. 若消耗完該批次，分配該批次全部剩餘成本，以吸收累積捨入差異。
8. 若任一事件找不到足夠庫存，整次重算失敗，回報食材、事件時間及缺少數量。
9. 餐點總成本為其所有 meal allocation 的成本總和；調整 allocation 不列入餐點成本。
10. 所有會影響歷史的新增、修改、刪除及還原，都必須在同一 IndexedDB transaction 中先執行重算，成功後才提交。
...
- 同日的日期型採買在早晨餐點中仍可正確取得庫存，前一日餐點則不可使用未來採買。
- 多批不同價格食材依 FIFO 正確扣料。
- 單次用量跨越多個批次時，數量與成本正確。
- 批次完全耗盡時不殘留捨入成本。
- 調整會扣除庫存成本，但不增加餐費。
