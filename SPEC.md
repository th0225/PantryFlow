...
2. 每項未捨入成本為：`實付總額 × 品項小計 / 全部品項小計`。
3. 先將每項成本向下取整至分，再依小數餘額由大至小分配剩餘分數。
4. 小數餘額相同時，以品項穩定 ID 排序。
5. 分攤完成後，所有 `allocatedCostCents` 加總必須精確等於 `paidTotalCents`。
...
- 核心單位、分攤及 FIFO 邏輯須設計為不依賴 React 或 IndexedDB 的純函式；資料服務負責在 transaction 中呼叫核心邏輯。
